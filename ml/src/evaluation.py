...
def evaluate_model(
    ...
    Args:
        ...
        y_test: Ground-truth / derived binary labels.
        model_type_name: Readable model identifier.
        training_samples: Number of training rows.
        data_source_note: Optional dataset / label description for metrics export.
    ...
    source_note = data_source_note or (
        "Trained on Synthea synthetic EHR encounters with a derived 30-day "
        "inpatient readmission label for software and UI demonstration."
    )
    ...
    metrics_payload = {
        ...
        "preprocessing_summary": [
            "Synthea inpatient encounters joined with patients, conditions, and payers",
            "Derived 30-day inpatient readmission label from encounter timelines",
            "StandardScaler (age, admission_count, days_since_last_admission) + OneHotEncoder (gender, primary_diagnosis, insurance, has_pcp)",
        ],
        "training_samples": int(training_samples) if training_samples else len(X_test) * 4,
        "test_samples": int(len(X_test)),
        "limitations": [
            source_note,
            "Label is derived from Synthea encounter timelines (next inpatient within 30 days), not a real-world clinical outcomes registry.",
        ...
        "notes": "Baseline explainable Logistic Regression evaluated on Synthea-derived holdout set.",
    }
